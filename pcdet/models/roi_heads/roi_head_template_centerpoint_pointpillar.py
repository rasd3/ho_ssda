...
from .target_assigner.proposal_target_layer_centerpoint_pointpillar import ProposalTargetLayer

def limit_period(val, offset=0.5, period=np.pi):
    return val - torch.floor(val / period + offset) * period

class RoIHeadTemplate_CenterPoint_PointPillar(nn.Module):
    ...
    def make_fc_layers(self, input_channels, output_channels, fc_list):
        ...
        for k in range(0, fc_list.__len__()):
            fc_layers.extend([
                nn.Conv1d(pre_channel, fc_list[k], kernel_size=1, bias=False), #ori
                #nn.Linear(pre_channel, fc_list[k], bias=False), #fc
        ...
        fc_layers.append(nn.Conv1d(pre_channel, output_channels, kernel_size=1, bias=True)) #ori
        #fc_layers.append(nn.Linear(pre_channel, output_channels, bias=True)) #fc
    ...
    def assign_targets(self, batch_dict):
        ...
        if rois.shape[-1] == 9:
            ...
            gt_of_rois[:, :, 7:-1] = common_utils.rotate_points_along_z(
    ...
    def get_box_reg_layer_loss(self, forward_ret_dict, scalar=True):
        ...
        batch_size = self.batch_size # TODO currently hardcoded
        ...
        if loss_cfgs.REG_LOSS == 'smooth-l1':
            ...
            # )
            reg_targets = gt_boxes3d_ct.view(rcnn_batch_size, code_size)
    ...
    def get_box_cls_layer_loss(self, forward_ret_dict, scalar=True):
        ...
        if loss_cfgs.CLS_LOSS == 'BinaryCrossEntropy':
            ...
            #batch_loss_cls = F.binary_cross_entropy(torch.softmax(rcnn_cls_flat, dim=0), rcnn_cls_labels.float(), reduction='none')
    ...
    def generate_predicted_boxes(self, batch_size, rois, cls_preds, box_preds):
        ...
        batch_box_preds= (batch_box_preds + local_rois).view(-1, code_size) #modification 1026
