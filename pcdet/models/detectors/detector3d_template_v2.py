...
class Detector3DTemplateV2(nn.Module):
    ...
    def build_dense_head(self, model_info_dict):
        ...
        dense_head_module = dense_heads.__all__[self.model_cfg.DENSE_HEAD.NAME](
            ...
            #voxel_size=self.model_cfg.DENSE_HEAD.VOXEL_SIZE
        )
    ...
    def post_processing(self, batch_dict, no_recall_dict=False, override_thresh=None, no_nms=False):
        ...
        for index in range(batch_size):
            ...
            if not isinstance(batch_dict['batch_cls_preds'], list):
                cls_preds = batch_dict['batch_cls_preds'][batch_mask] #in
            ...
            if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS: #not
                ...
                for cur_cls_preds, cur_label_mapping in zip(cls_preds, multihead_label_mapping):
                    ...
                    cur_pred_labels = cur_label_mapping[cur_pred_labels] +1
    ...
    def reorder_rois_for_refining(self, batch_size, pred_dicts): #, bev_feature):
    ...
    def reorder_rois_for_nuscenes_eval(self, batch_size, pred_dicts): #, bev_feature):
        num_max_rois = max([len(cur_dict['pred_boxes']) for cur_dict in pred_dicts])
        num_max_rois = max(1, num_max_rois)  # at least one faked rois to avoid error
        #num_max_rois = 500
        pred_boxes = pred_dicts[0]['pred_boxes']
        ...
        for bs_idx in range(batch_size):
            ...
            record_dict = {
                'pred_boxes': rois,
                'pred_scores': roi_scores,
                'pred_labels': roi_labels,
            }
            pred_dicts_.append(record_dict)
    ...
    def post_process(self, batch_dict): #modification 1026
        batch_size = batch_dict['batch_size']
        pred_dicts = [] 
    ...
    def post_processing_for_roi__(self, batch_dict):
        post_process_cfg = self.model_cfg.POST_PROCESSING
        batch_size = batch_dict['batch_size']
    ...
    def post_processing_for_roi_ssl_(self, batch_dict):
        post_process_cfg = self.model_cfg.POST_PROCESSING
        batch_size = batch_dict['batch_size']
        final_pred_dict = [{'pred_boxes':batch_dict["rois"][i], 'pred_scores':batch_dict['roi_scores'][i], 'pred_labels':batch_dict['roi_labels'][i], "pred_sem_scores":torch.sigmoid(batch_dict['batch_cls_preds'][i])} for i in range(batch_size)]
        recall_dict = {}
        for index in range(batch_size):
            pred_boxes = final_pred_dict[index]['pred_boxes']
    ...
    def post_processing_for_nuscenes(self, batch_dict):
        post_process_cfg = self.model_cfg.POST_PROCESSING
        batch_size = batch_dict['batch_size']
        #final_pred_dict = batch_dict['final_box_dicts']
        final_pred_dict = []
        for ii in range(batch_size):
            non_zero_mask = torch.nonzero(batch_dict['roi_labels'][ii], as_tuple=True)
            if len(non_zero_mask[0]) == 0:
                print("zero")
            
            if len(non_zero_mask[0]) > 500:
                print("over 500")
                score, idx = batch_dict['roi_scores'][ii].clone().sort(descending=True)
                idx = idx[:500]
                data = {'pred_boxes':batch_dict["rois"][ii][idx], 'pred_scores':batch_dict['roi_scores'][ii][idx], 'pred_labels':batch_dict['roi_labels'][ii][idx]}
            else:
                data = {'pred_boxes':batch_dict["rois"][ii][non_zero_mask[0]], 'pred_scores':batch_dict['roi_scores'][ii][non_zero_mask[0]], 'pred_labels':batch_dict['roi_labels'][ii][non_zero_mask[0]]}
            print(data['pred_boxes'].shape)


            final_pred_dict.append(data)
