...
def create_part_dbinfos(version,
    ...
    print('%s: train scene(%d), unlabeled scene(%d), val scene(%d)' %
          (version, len(train_scenes), len(unlabeled_scenes), len(val_scenes)))

    train_nusc_infos, unlabeled_nusc_infos, val_nusc_infos = nuscenes_utils.fill_trainval_part_infos(
        data_path=data_path,
        nusc=nusc,
        train_scenes=train_scenes,
    ...
    if version == 'v1.0-test':
        ...
        with open(
                save_path /
                f'nuscenes_infos_{max_sweeps}sweeps_unlabeled_{part_split}.pkl',
                'wb') as f:
...
if __name__ == '__main__':
    ...
    if args.func == 'create_nuscenes_infos':
        ...
        data_save_path = ROOT_DIR / 'data' / 'nuscenes'
