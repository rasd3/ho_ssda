...
class NuScenesDatasetSSL(DatasetTemplate):
    ...
    def __len__(self):
        if self._merge_all_iters_to_one_epoch:
            return len(self.labeled_infos) * self.total_epochs

        if self.training:
            return len(self.labeled_infos) * self.repeat
        else:
            return len(self.labeled_infos)
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            ...
            if not self.dataset_cfg.PRED_VELOCITY and 'gt_boxes_ema' in unlabeled_data_dict:
                unlabeled_data_dict['gt_boxes_ema'] = unlabeled_data_dict[
                    'gt_boxes_ema'][:, [0, 1, 2, 3, 4, 5, 6, -1]]
...
class NuScenesDatasetSSLSE(NuScenesDatasetSSL):
    def __len__(self):
        if self._merge_all_iters_to_one_epoch:
            return len(self.labeled_infos) * self.total_epochs
        if self.training:
            return len(self.labeled_infos) + len(self.unlabeled_infos)
        else:
            return len(self.labeled_infos)

    def __getitem__(self, index):
        index = index % (len(self.labeled_infos) + len(self.unlabeled_infos))

        if self.training:
            data_dict_list = []
            for _ in range(2):
                index = np.random.choice(len(self.unlabeled_infos) + len(self.labeled_infos), 1)[0]
                if index < len(self.labeled_infos):
                    info = copy.deepcopy(self.labeled_infos[index])
                    points = self.get_lidar_with_sweeps(
                        info, max_sweeps=self.dataset_cfg.MAX_SWEEPS)
                    if self.shift_coor:
                    ...
                    if not self.dataset_cfg.PRED_VELOCITY and 'gt_boxes_ema' in data_dict:
                        data_dict['gt_boxes_ema'] = data_dict[
                            'gt_boxes_ema'][:, [0, 1, 2, 3, 4, 5, 6, -1]]
                    data_dict['mask'] = np.array([1.])

                    data_dict_list.append(data_dict)
                else:
                    index_unlabeled = index - len(self.labeled_infos)
                    info_unlabeled = copy.deepcopy(
                        self.unlabeled_infos[index_unlabeled])
                    unlabeled_points = self.get_lidar_with_sweeps(
                        info_unlabeled, max_sweeps=self.dataset_cfg.MAX_SWEEPS)
                    ...
                    if not self.dataset_cfg.PRED_VELOCITY and 'gt_boxes_ema' in unlabeled_data_dict:
                        unlabeled_data_dict['gt_boxes_ema'] = unlabeled_data_dict[
                            'gt_boxes_ema'][:, [0, 1, 2, 3, 4, 5, 6, -1]]
                    unlabeled_data_dict['mask'] = np.array([0.])
                    data_dict_list.append(unlabeled_data_dict)
    ...
    def collate_batch(batch_list, _unused=False):
        ...
        if isinstance(batch_list[0], list):
            for cur_sample in batch_list:
                for key, val in cur_sample[0].items():
                    data_dict[key].append(val)
                if 'mask' not in cur_sample[0]:
                    data_dict['mask'].append(np.ones([len(batch_list)]))
                for key, val in cur_sample[1].items():
                    data_dict[key].append(val)
                if 'mask' not in cur_sample[1]:
                    data_dict['mask'].append(np.zeros([len(batch_list)]))
            batch_size = len(batch_list) * 2
        else:
            mask = data_dict.pop('mask')
            for cur_sample in batch_list:
                for key, val in cur_sample.items():
                    data_dict[key].append(val)
