...
class WaymoDatasetSSL(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names=None, training=True, root_path=None, logger=None):
        ...
        if self.training:
            unlabeled_dir = self.root_path / 'ImageSets' / ('unlabeled' + self.split[5:] + '.txt')
            self.unlabeled_list = [x.strip() for x in open(unlabeled_dir).readlines()]
            self.unlabeled_infos = []
            self.seq_name_to_unlabeled_infos = self.include_unlabeled_data(self.mode)
    ...
    def include_unlabeled_data(self, mode):
        self.logger.info('Loading Waymo dataset')
        waymo_infos = []
        seq_name_to_infos = {}

        num_skipped_infos = 0
        for k in range(len(self.unlabeled_list)):
            sequence_name = os.path.splitext(self.unlabeled_list[k])[0]
        ...
        self.unlabeled_infos.extend(waymo_infos[:])
        self.logger.info('Total skipped unlabeled info %s' % num_skipped_infos)
        self.logger.info('Total samples for unlabeled Waymo dataset: %d' % (len(waymo_infos)))

        if self.dataset_cfg.SAMPLED_INTERVAL[mode] > 1:
            sampled_waymo_infos = []
            for k in range(0, len(self.unlabeled_infos), self.dataset_cfg.SAMPLED_INTERVAL[mode]):
                sampled_waymo_infos.append(self.unlabeled_infos[k])
            self.unlabeled_infos = sampled_waymo_infos
            self.logger.info('Total sampled samples for unlabeled Waymo dataset: %d' % len(self.unlabeled_infos))
    ...
    def __len__(self):
        ...
        if self.training:
            return len(self.infos) * self.repeat
        else:
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            ...
            pc_info_unlabeled = info_unlabeled['point_cloud']
            sequence_name_unlabeled = pc_info_unlabeled['lidar_sequence']
            sample_idx_unlabeled = pc_info_unlabeled['sample_idx']
            input_dict_unlabeled = {
                'sample_idx': sample_idx_unlabeled
            }
            if self.use_shared_memory and index_unlabeled < self.shared_memory_file_limit:
                sa_key = f'{sequence_name_unlabeled}___{sample_idx_unlabeled}'
                points_unlabeled = SharedArray.attach(f"shm://{sa_key}").copy()
            else:
                points_unlabeled = self.get_lidar(sequence_name_unlabeled, sample_idx_unlabeled)

            if self.dataset_cfg.get('SEQUENCE_CONFIG', None) is not None and self.dataset_cfg.SEQUENCE_CONFIG.ENABLED:
                points_unlabeled, num_points_all_unlabeled, sample_idx_pre_list_unlabeled, poses_unlabeled, 
                pred_boxes_unlabeled, pred_scores_unlabeled, pred_labels_unlabeled = self.get_sequence_data(
                    info_unlabeled, points_unlabeled, sequence_name_unlabeled, sample_idx_unlabeled, self.dataset_cfg.SEQUENCE_CONFIG,
                    load_pred_boxes=self.dataset_cfg.get('USE_PREDBOX', False)
                )
                input_dict_unlabeled['poses'] = poses_unlabeled
                if self.dataset_cfg.get('USE_PREDBOX', False):
                    input_dict_unlabeled.update({
                        'roi_boxes': pred_boxes_unlabeled,
                        'roi_scores': pred_scores_unlabeled,
                        'roi_labels': pred_labels_unlabeled,
                    })

            input_dict_unlabeled.update({
                'points': points_unlabeled,
                'frame_id': info_unlabeled['frame_id'],
            })

            if 'annos' in info_unlabeled:
                annos_unlabeled = info_unlabeled['annos']
                annos_unlabeled = common_utils.drop_info_with_name(annos_unlabeled, name='unknown')

                if self.dataset_cfg.get('INFO_WITH_FAKELIDAR', False):
                    gt_boxes_lidar_unlabeled = box_utils.boxes3d_kitti_fakelidar_to_lidar(annos_unlabeled['gt_boxes_lidar'])
                else:
                    gt_boxes_lidar_unlabeled = annos_unlabeled['gt_boxes_lidar']

                if self.dataset_cfg.get('TRAIN_WITH_SPEED', False):
                    assert gt_boxes_lidar_unlabeled.shape[-1] == 9
                else:
                    gt_boxes_lidar_unlabeled = gt_boxes_lidar_unlabeled[:, 0:7]

                if self.training and self.dataset_cfg.get('FILTER_EMPTY_BOXES_FOR_TRAIN', False):
                    mask_unlabeled = (annos_unlabeled['num_points_in_gt'] > 0)  # filter empty boxes
                    annos_unlabeled['name'] = annos_unlabeled['name'][mask_unlabeled]
                    gt_boxes_lidar_unlabeled = gt_boxes_lidar_unlabeled[mask_unlabeled]
                    annos_unlabeled['num_points_in_gt'] = annos_unlabeled['num_points_in_gt'][mask_unlabeled]

                input_dict_unlabeled.update({
                    'gt_names': annos_unlabeled['name'],
                    'gt_boxes': gt_boxes_lidar_unlabeled,
                    'num_points_in_gt': annos_unlabeled.get('num_points_in_gt', None)
                })

            data_dict_unlabeled = self.prepare_data(data_dict=input_dict_unlabeled)
            data_dict_unlabeled['metadata'] = info_unlabeled.get('metadata', info_unlabeled['frame_id'])
            data_dict_unlabeled.pop('num_points_in_gt', None)

            return [data_dict, data_dict_unlabeled]
...
def create_waymo_part_infos(dataset_cfg, class_names, data_path, save_path,
    ...
    train_split = 'train_' + split
    unlabeled_split = 'unlabeled_' + split

    train_filename = save_path / ('%s_infos_%s.pkl' % (processed_data_tag, train_split))
    unlabeled_filename = save_path / ('%s_infos_%s.pkl' % (processed_data_tag, unlabeled_split))
    ...
    dataset.set_split(unlabeled_split)
    waymo_infos_unlabeled = dataset.get_infos(
        raw_data_path=data_path / raw_data_tag,
        save_path=save_path / processed_data_tag, num_workers=workers, has_label=True,
        sampled_interval=1, update_info_only=update_info_only
    )
    with open(unlabeled_filename, 'wb') as f:
        pickle.dump(waymo_infos_unlabeled, f)
    print('----------------Waymo info unlabeled file is saved to %s----------------' % unlabeled_filename)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--func', type=str, default='create_part_infos', help='')
    ...
    if args.func == 'create_part_infos':
        try:
