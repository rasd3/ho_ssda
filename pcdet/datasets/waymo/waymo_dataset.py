...
class WaymoDataset(DatasetTemplate):
    ...
    def __getitem__(self, index):
        ...
        if 'annos' in info:
            ...
            # adjust source class name to target class name
            if False:
                if self.dataset_cfg.get('WAYMO_TO_NUSCENES_ANNO', False):
                    annos = common_utils.waymo_to_nuscenes_anno(annos)
                if self.dataset_cfg.get('WAYMO_TO_KITTI_ANNO', False):
                    annos = common_utils.waymo_to_kitti_anno(annos)
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        def kitti_eval(eval_det_annos, eval_gt_annos):
            from ..kitti.kitti_object_eval_python import eval_waymo as kitti_eval
