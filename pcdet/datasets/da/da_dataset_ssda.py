...
class DADatasetCO(torch_data.Dataset):
    ...
    def __init__(self, dataset_cfg, class_names=None, training=True, root_path=None, logger=None):
        ...
        self.class_names = self.trg_dataset.class_names
    ...
    def kitti_eval(self, det_annos, class_names, **kwargs):
        return self.trg_dataset.kitti_eval(det_annos, class_names, **kwargs)
...
__all__ = {
    ...
    'NuScenesDatasetSSLSE': NuScenesDatasetSSLSE,
...
class DADatasetDANN(DADatasetSSDA):
    ...
    def __len__(self):
        if self.training:
            return len(self.src_dataset) * self.repeat
        else:
            if self.target:
                return len(self.trg_dataset)
            else:
    ...
    def __getitem__(self, index):
        if self.training:
            ...
            trg_item = self.trg_dataset.__getitem__(index)
            if 'domain' not in src_item:
                src_item['domain'] = 0
            if 'domain' not in trg_item:
                trg_item['domain'] = 1
...
class DADatasetSSDASER(DADatasetSSDA):
    def __len__(self):
        if self.training:
            return len(self.trg_dataset)
        else:
