    ...
    def __getitem__(self, index):
        ...
        if prob < self.dataset_cfg.CUTMIX_PROB:
            ...
            if self.dataset_cfg['NuScenesDataset'].get('SHIFT_COOR', None):
                nus_input_dict['points'][:, 0:3] += np.array(self.dataset_cfg['NuScenesDataset'].SHIFT_COOR, dtype=np.float32)
                nus_input_dict['shift_coor'] = np.array(self.dataset_cfg['NuScenesDataset'].SHIFT_COOR, dtype=np.float32)
