...
def train_one_epoch_dann(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    mgfa_loss_weight = optim_cfg.get('MGFA_LOSS_WEIGHT', 1.0)
    mgfa_loss_func = optim_cfg.get('MGFA_LOSS', 'MMD')
    if type(model) == torch.nn.parallel.distributed.DistributedDataParallel:
            ...
            for idx in range(len(src_mgfa_feats)):
                if mgfa_loss_func == 'MMD':
                    loss_mgfa_i = mmd.mix_rbf_mmd2(src_mgfa_feats[idx].detach(), 
                                                   trg_mgfa_feats[idx],
                                                   sigma_list)
                elif mgfa_loss_func == 'MSE':
                    loss_func = torch.nn.MSELoss()
                    loss_mgfa_i = loss_func(trg_mgfa_feats[idx],
                                            src_mgfa_feats[idx].detach())
                elif mgfa_loss_func == 'KL':
                    loss_func = torch.nn.KLDivLoss(reduction='batchmean')
                    pred = F.log_softmax(trg_mgfa_feats[idx], dim=1)
                    targ = F.softmax(src_mgfa_feats[idx].detach(), dim=1)
                    loss_mgfa_i = loss_func(pred, targ)
                else:
